...
app.use(
  '/dist',
  express.static(path.join(process.cwd(), 'dist'), { maxAge: '30d', })
);
...
app.get('/*', (req, res) => {
  ...
  const options = {
    ssrHtml: ReactDOMServer.renderToString(<div>test</div>),
  };
  ...
  const html = tpl.replace('<!-- STATIC_DOM -->', options.ssrHtml);
